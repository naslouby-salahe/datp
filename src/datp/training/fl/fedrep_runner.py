...
"""FedRep-AE stress-test runner: shared encoder + local decoder heads.

FedRep-AE is a locked stress test — it is never added to the Baseline enum.
This is NOT Ditto. See T18 / PRE_CODING_PLAN §6.4 for the choice documentation.
"""
...
def run_fedrep_training(
    ...
    *,
    ...
    Each client personalizes its decoder locally.
    """
    ...
    model = _build_model(cfg)
    model.to(device)
    # Initial parameters: encoder only for FedRep.
    initial_params = get_parameters(model.encoder)
    ...
    ckpt_dir = base_dir / "fedrep" / f"seed_{seed}"
    ...
    # Restore the shared encoder from final aggregated params.
    final_params = strategy.latest_parameters
    if final_params is not None:
        set_parameters(model.encoder, final_params)
