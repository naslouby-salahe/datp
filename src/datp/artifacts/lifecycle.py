...
from datp.artifacts.names import RunMarker, RunState
...
def check_run_state(run_dir: Path) -> RunState:
    ...
    has_in_progress = (run_dir / RunMarker.IN_PROGRESS).exists()
    has_done = (run_dir / RunMarker.DONE).exists()
    has_aborted = (run_dir / RunMarker.ABORTED).exists()
...
class RunLifecycle:
    ...
    def __enter__(self) -> "RunLifecycle":
        ...
        (self.run_dir / RunMarker.ABORTED).unlink(missing_ok=True)
        (self.run_dir / RunMarker.IN_PROGRESS).touch()
        return self
    ...
    def __exit__(
        ...
        in_progress_path = self.run_dir / RunMarker.IN_PROGRESS

        if exc_type is None:
            in_progress_path.unlink(missing_ok=True)
            (self.run_dir / RunMarker.DONE).write_text("Run completed successfully.\n")
        else:
            in_progress_path.unlink(missing_ok=True)
            try:
    ...
    def _write_aborted(
        ...
        (self.run_dir / RunMarker.ABORTED).write_text("\n".join(lines) + "\n")
