...
def build_invariant_results(
    invariant_inputs: dict[tuple[Regime, int, str | None], dict[Baseline, dict[str, str]]],
    score_hashes_by_cell: dict[tuple[Regime, int, str | None], dict[Baseline, dict[tuple[str, str], str]]],
    ...
    for key, by_baseline in sorted(invariant_inputs.items()):
        ...
        required = [b for b in controlled_baselines if not (b == Baseline.B3 and regime != Regime.A)]
        ...
        model_shared = len({by_baseline[b]["model_hash"] for b in checked} | {by_baseline[b]["encoder_hash"] for b in checked}) <= 1
        ...
        per_baseline_hashes = score_hashes_by_cell[key] if key in score_hashes_by_cell else {}
        checked_score_maps = [per_baseline_hashes[b] for b in checked if b in per_baseline_hashes]
        ...
        invariant_results.append(BaselineInvariantResult(
            regime=regime,
            seed=seed,
            alpha=alpha_text,
            status=status,
