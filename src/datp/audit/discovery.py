...
def iter_score_cells(base_dir: Path) -> list[ScoreCellLocation]:
    ...
    for manifest_path in sorted(scores_root.glob(f"*/{SEED_PREFIX}*/{SCORING_MANIFEST_FILE}")):
        cells.append(_parse_score_cell(scores_root, manifest_path.parent))
    for manifest_path in sorted(scores_root.glob(f"*/{SEED_PREFIX}*/{ALPHA_PREFIX}*/{SCORING_MANIFEST_FILE}")):
        cells.append(_parse_score_cell(scores_root, manifest_path.parent))
    return cells
