...
class AbsorptionTable(FrozenModel):
...
def _derive_threshold_for_stress(
    ...
    tau_global, _ = derive_tau_global(
        client_cal_errors, regime=regime, threshold_cfg=threshold_cfg
    )
...
def write_absorption_table(table: AbsorptionTable, output_dir: Path) -> None:
    """Write absorption table as CSV and JSON."""
    output_dir.mkdir(parents=True, exist_ok=True)
    csv_path = output_dir / ABSORPTION_TABLE_CSV
    fieldnames = list(AbsorptionRow.model_fields)
    with csv_path.open("w", newline="", encoding="utf-8") as fh:
        writer = csv.DictWriter(fh, fieldnames=fieldnames)
        writer.writeheader()
        for row in table.rows:
            writer.writerow(row.model_dump(mode="json"))
    (output_dir / ABSORPTION_TABLE_JSON).write_text(
