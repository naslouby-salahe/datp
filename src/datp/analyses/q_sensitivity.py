...
def _config_for_cell(cfg: DatpConfig | None) -> DatpConfig:
    return cfg if cfg is not None else compose_analysis_config()
        ...
        for q in q_grid:
            ...
            for baseline in _Q_SENSITIVITY_BASELINES:
                ...
                evaluation = evaluate_threshold_result(
                    threshold_result, score_provider, regime, seed, alpha_f
                )
                rows.append(
                    QSensitivityRow(
                        q=q,
...
def _write_csv(result: QSensitivityResult, analysis_dir: Path) -> None:
    ...
    with (analysis_dir / Q_SENSITIVITY_TABLE_CSV).open(
...
def _write_heatmap(result: QSensitivityResult, analysis_dir: Path, plt) -> None:  # type: ignore[type-arg]
    ...
    for ax_idx, regime in enumerate(all_regimes):
        ...
        ax.set_yticklabels(
            [baseline_labels.get(bl, bl.value) for bl in baselines_present], fontsize=8
        )
