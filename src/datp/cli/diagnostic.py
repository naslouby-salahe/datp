...
def diagnostic(
    raw_dir: Annotated[
        Path | None, typer.Option(help="Path to raw N-BaIoT data")
    ] = None,
    ...
    actual_raw_dir = (
        ...
        else raw_root(DatasetID.NBAIOT, base_dir=data_root)
    )
...
def diagnostic_b(
    raw_dir: Annotated[
        Path | None, typer.Option(help="Path to raw CICIoT2023 data")
    ] = None,
    output_dir: Path = typer.Option(..., help=_OUTPUT_DIR_HELP),
    data_root: _DataRoot = DEFAULT_BASE_DIR,
    seed: _Seed = None,
    ...
    actual_raw_dir = (
        raw_dir
        if raw_dir is not None
        else raw_root(DatasetID.CICIOT2023, base_dir=data_root)
    )
...
def diagnostic_c(
    ...
    seed: _Seed = None,
    alpha: Annotated[
        float | None, typer.Option(help="Dirichlet concentration parameter")
    ] = None,
    skip_prepare: bool = typer.Option(
        False, _SKIP_PREPARE_FLAG, help=_SKIP_PREPARE_HELP
    ),
    ...
    actual_alpha = (
        alpha if alpha is not None else BASE_CONFIG.experiment.regime_c_alphas[0]
    )
    ...
    prepared_dir = prepared_root_for_regime(
        Regime.C, base_dir=data_root, alpha=actual_alpha, seed=actual_seed
    )
