...
with benign distributions: when a client has a noisy benign tail, raising its threshold
reduces benign false alarms but can also swallow low-magnitude anomaly scores,
reducing TPR or lower-tail Macro-F1 (Fig.~\ref{fig:mechanism} illustrates the
benign-side mechanism).
...
Because Regime~A contains only nine physical devices, B4 should be interpreted as an
exploratory taxonomy-free intermediate policy rather than a validated general clustering
rule; validation on larger physical-device fleets is required before deployment generalization.
...
Regime~B uses file-defined CICIoT2023 pseudo-clients (dataset-file partitions, not physical IoT devices) with randomized within-file ordering that does not preserve temporal drift.
Regime~B is therefore interpreted only as a randomized file-level pseudo-client check, not as a temporally realistic CICIoT2023 deployment boundary.
Under this specific partition, benign reconstruction-error distributions are near-homogeneous (mean JS divergence $0.004$, max $0.038$); alternative partitions (e.g., by attacker identity, attack type, or physical device) preserving chronological ordering could yield different heterogeneity and remain future work.
