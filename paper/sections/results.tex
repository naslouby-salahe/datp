...
Each of the five planned seeds gives a positive B1--B2 delta, with mean
$\Delta = 0.718$, SD$\,{=}\,0.079$, and range $[0.588,\, 0.772]$
(Table~\ref{tab:per_seed}); these are descriptive summaries, not confidence intervals.
...
The absolute reduction of 0.718 corresponds to a relative reduction
...
Table~\ref{tab:regime_a} shows the detection-uniformity tradeoff:
...
\begin{figure}[t]
  ...
  \caption{Per-client FPR distributions under B1, B2, and B4 (Regime~A, pooled over 5 seeds).
           The plot is descriptive; inference uses the seed-level deltas reported in the text.}
...
B4 requires no device taxonomy and transmits only the 4-scalar fingerprint per client;
...
so strict monotonicity between these two levels should not be inferred from these data;
