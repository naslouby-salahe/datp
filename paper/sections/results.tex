...
a relative reduction of $70.6\%$ ($= (\text{CV}_{\text{B1}} - \text{CV}_{\text{B2}})/\text{CV}_{\text{B1}}$),
achieved with zero additional threshold uplink
since each client computes and applies its own threshold locally.
B2 does not require uploading a threshold, but it does require local threshold state,
versioning with the model and calibration window, and sufficient benign calibration data.
...
these are reported descriptively rather than as population-level inference from five seeds.
...
Regime~B uses file-defined CICIoT2023 pseudo-clients (not physical devices) with randomized ordering; it is interpreted only as a near-homogeneous applicability boundary under this partition.
All runs were budget-capped at 150 rounds; comparisons remain controlled because all policies share the same frozen AE per seed and mean Macro-F1 is stable at ${\approx}0.959$ across strategies.
