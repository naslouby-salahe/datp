...
because they can be trained on benign traffic alone, without attack labels.
...
Across heterogeneous device types, however, benign reconstruction-error
distributions differ, so a single shared $\tau$ imposes unequal
...
The study uses N-BaIoT~\cite{meidan2018n}, benign and botnet traffic captured from
nine commercial IoT devices, as the primary confirmatory regime (Regime~A);
CICIoT2023~\cite{neto2023ciciot2023}, a large-scale IoT attack dataset partitioned here
into randomized file-level pseudo-clients, as an applicability check under
