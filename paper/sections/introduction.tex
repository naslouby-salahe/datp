...
We ask whether calibration scope alone can reduce per-client FPR dispersion
...
This controlled-comparison gap motivates our study.
...
Five random seeds are used; seed-level bootstrap stability intervals summarize consistency.
The FL simulation uses Flower~\cite{beutel2020flower}.

\textbf{Contributions.}
\begin{itemize}
  \item A controlled threshold-calibration study in which the FedAvg AE,
        training protocol, and seeds are held fixed across B1--B4; only the post-training
        threshold scope varies.
  \item Empirical evidence on N-BaIoT that the client-averaged shared threshold (B1) yields
        CV(FPR)\,$\approx 1.02$; per-client calibration (B2) reduces it to $0.299$
        ($\Delta = 0.718$, seed-level bootstrap interval $[0.647,\,0.769]$, all five seed deltas positive).
        Detection-quality tradeoffs---mean Macro-F1 improves but lower-tail P10 Macro-F1 worsens---are
        characterized and should be weighed against the FPR-equity gain.
  \item Applicability conditions: under our CICIoT2023 file-level partition (pseudo-clients,
        not physical devices), benign distributions are near-homogeneous and per-client
        calibration offers no improvement.
        A Dirichlet severity sweep is consistent with the benefit concentrating in
        high-heterogeneity regimes.
\end{itemize}
