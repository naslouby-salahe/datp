...
class TestExperimentConfig:
    def test_valid_construction(self) -> None:
        e = ExperimentConfig(
            ...
            stress_test_fedprox_mu_grid=[0.0, 0.1, 1.0],
...
class TestAnalysisConfig:
    def test_valid_construction(self) -> None:
        a = AnalysisConfig(
            ...
            cal_sweep_n_cal=[50, 100, 250, 500, 1000, 5000],
            cal_sweep_n_repeats=100,
            cal_sweep_seed_base=20260524,
            tau_shrink_lambdas=[0.0, 0.25, 0.5, 0.75, 1.0],
            fedstats_k_min=0.0,
            fedstats_k_max=5.0,
            fedstats_k_step=0.01,
            fedstats_target_exceedance=0.05,
        )
        assert a.cal_sweep_n_repeats == 100

    def test_extra_forbidden(self) -> None:
        with pytest.raises(ValidationError, match="extra"):
            AnalysisConfig(  # type: ignore[call-arg]
                q_grid=[0.90, 0.95],
                cal_sweep_n_cal=[50, 100],
                cal_sweep_n_repeats=100,
                cal_sweep_seed_base=20260524,
                tau_shrink_lambdas=[0.0, 0.5, 1.0],
                fedstats_k_min=0.0,
                fedstats_k_max=5.0,
                fedstats_k_step=0.01,
                fedstats_target_exceedance=0.05,
