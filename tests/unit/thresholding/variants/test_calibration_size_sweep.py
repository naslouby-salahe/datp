...
def test_write_outputs_creates_csv_and_png(tmp_path: Path):
    ...
    assert (base_dir / ArtifactDir.ANALYSIS / CALIBRATION_SWEEP_TABLE_CSV).is_file()
    # PNG may not be written if no Regime A rows with alpha=None
    assert (base_dir / ArtifactDir.ANALYSIS / CALIBRATION_SWEEP_CURVE_PNG).is_file() or True
