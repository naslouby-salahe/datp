...
class TestEnforceRegimeHappyPath:
    ...
    def test_all_regimes_d_passes(self) -> None:
        assert _all_regimes(regime=Regime.D) == "all:d"
...
class TestEnforceRegimeDisallowed:
    ...
    def test_single_disallowed_d_raises_value_error(self) -> None:
        with pytest.raises(ValueError, match="restricted to"):
            _single_a(regime=Regime.D)

    def test_multi_disallowed_c_raises_value_error(self) -> None:
        with pytest.raises(ValueError, match="restricted to"):
            _multi_ab(regime=Regime.C)

    def test_multi_disallowed_d_raises_value_error(self) -> None:
        with pytest.raises(ValueError, match="restricted to"):
            _multi_ab(regime=Regime.D)
